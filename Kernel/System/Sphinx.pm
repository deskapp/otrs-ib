...
sub new {
    ...
    $Self->{Config} = $Kernel::OM->Get('Kernel::Config')->Get('Ticket::SearchIndex::Sphinx');
    if ( !$Self->{Config} ) {
...
sub Do {
    ...
    # check needed stuff
    if ( !$Param{SphinxQL} ) {
        $Kernel::OM->Get('Kernel::System::Log')->Log( Priority => 'error', Message => 'Need SphinxQL!' );
        return;
    }
...
sub FetchrowArray {
    ...
    for my $Element (@Row) {
        ...
        if ( !defined $Self->{Encode} || ( $Self->{Encode} && $Self->{Encode}->[$Counter] ) ) {
            $Kernel::OM->Get('Kernel::System::Encode')->EncodeInput( \$Element );
        }
    }
    continue {
        $Counter++;
